...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_settings'])) {
    ...
    if ($interval <= 0 || $minAmount <= 0 || $maxAmount <= 0 || $minAmount > $maxAmount) {
        ...
        $updateSql = "UPDATE settings SET claim_interval_minutes = $interval, min_amount = $minSql, max_amount = $maxSql, faucet_active = $active WHERE id = 1";
...
<html lang="cs">
<head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>TurboSol Admin Panel</title>
...
<div class="container py-4">
    ...
    <div class="row g-4">
        <div class="col-lg-6">
            <div class="card shadow-sm"><div class="card-header fw-semibold">Nastavení faucetu</div><div class="card-body">
                <form method="POST" action="admin.php" class="d-grid gap-3">
                    <input type="hidden" name="update_settings" value="1">
                    <div><label class="form-label">Claim interval (minuty)</label><input type="number" class="form-control" name="claim_interval_minutes" min="1" value="<?php echo (int)$settings['claim_interval_minutes']; ?>" required></div>
                    ...
                    <div class="form-check"><input class="form-check-input" type="checkbox" name="faucet_active" id="faucet_active" <?php echo ((int)$settings['faucet_active'] === 1) ? 'checked' : ''; ?>><label class="form-check-label" for="faucet_active">Faucet aktivní</label></div>
        ...
        <div class="col-lg-6"><div class="row g-3">
            <div class="col-12"><div class="card shadow-sm"><div class="card-body"><h2 class="h6 text-muted mb-1">Počet claimů</h2><div class="fs-4 fw-bold"><?php echo $statsClaimsCount; ?></div></div></div></div>
            <div class="col-12"><div class="card shadow-sm"><div class="card-body"><h2 class="h6 text-muted mb-1">Celkem odesláno (SOL)</h2><div class="fs-4 fw-bold"><?php echo $statsClaimsSum; ?></div></div></div></div>
            <div class="col-12"><div class="card shadow-sm"><div class="card-body"><h2 class="h6 text-muted mb-1">Počet uživatelů</h2><div class="fs-4 fw-bold"><?php echo $statsUsersCount; ?></div></div></div></div>
            <div class="col-12"><div class="card shadow-sm"><div class="card-body"><h2 class="h6 text-muted mb-1">Nejvyšší level</h2><div class="fs-4 fw-bold"><?php echo $statsMaxLevel; ?></div></div></div></div>
        </div></div>
    </div>

    <div class="card shadow-sm mt-4"><div class="card-header fw-semibold">Posledních 10 claimů</div><div class="card-body p-0"><div class="table-responsive"><table class="table table-striped mb-0"><thead><tr><th>ID</th><th>User ID</th><th>Email</th><th>Level</th><th>Amount (SOL)</th><th>Status</th><th>Created</th></tr></thead><tbody>
    <?php if (count($recentClaims) === 0): ?><tr><td colspan="7" class="text-center text-muted py-3">Zatím žádné claimy.</td></tr><?php else: ?>
